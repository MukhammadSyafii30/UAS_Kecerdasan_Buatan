...
class Penjualan():
    ...
    def inferensi(self, jumlah_permintaan, jumlah_persediaan):
        ...
        α1 = min(rqt.turun(jumlah_permintaan), rsa.banyak(jumlah_persediaan))
        z1 = self.kurang(α1)
        result.append((α1, z1))

        # [R2] JIKA Permintaan TURUN, dan Persediaan SEDIKIT, 
        #     MAKA Penjualan Barang BERKURANG.
        α2 = min(rqt.turun(jumlah_permintaan), rsa.sedikit(jumlah_persediaan))
        z2 = self.kurang(α2)
        result.append((α2, z2))

        # [R3] JIKA Permintaan NAIK, dan Persediaan BANYAK, 
        #     MAKA Penjualan Barang BERTAMBAH.
        α3 = min(rqt.naik(jumlah_permintaan), rsa.banyak(jumlah_persediaan))
        z3 = self.tambah(α3)
        result.append((α3, z3))

        # [R4] JIKA Permintaan NAIK, dan Persediaan SEDIKIT,
        #     MAKA Penjualan Barang BERTAMBAH * 2
        α4 = min(rqt.naik(jumlah_permintaan), rsa.sedikit(jumlah_persediaan))
        z4 = self.tambah(α4) * 2
        result.append((α4, z4))

        # [R5] JIKA Permintaan NAIK, dan Persediaan CUKUP,
        #     MAKA Penjualan Barang BERKURANG / 2
        α5 = min(rqt.naik(jumlah_permintaan), rsa.cukup(jumlah_persediaan))
        z5 = self.kurang(α5) / 2
        result.append((α5, z5))

        # [R6] JIKA Permintaan NAIK, dan Persediaan CUKUP,
        #     MAKA Penjualan Barang BERTAMBAH.
        α6 = min(rqt.turun(jumlah_permintaan), rsa.cukup(jumlah_persediaan))
        z6 = self.tambah(α6)
        result.append((α6, z6))
